...
def infinite_well(steps=2000):
    ...
    # create plot
    infinite_well_plot(
        E, V, xvec, W, steps, n, Case, ask_to_save=True, ask_squared=True
    )
    # print output
    output(
        Case, ["Well Width (a.u.)", "Number of wavefunctions to plot"], [W * 2, n], E, n
    )
...
def double_finite_well_equal_depth(steps=2000):
    ...
    output(
        Case,
        ["Well Widths (a.u.)", "Well Depths (a.u.)", "Well Separation (a.u.)"],
        [W, -D, B * 2],
        E,
        n,
    )
...
def Kronig_Penney(steps=2000):
    ...
    for n in range(1, ((num_wells // 2) + 1)):
        U += step_func(xvec + W + (n * B) + ((n) * A)) - step_func(
            xvec + W + (n * B) + ((n - 1) * A)
        )
        U += step_func(xvec - W - (n * B) - ((n - 1) * A)) - step_func(
            xvec - W - (n * B) - ((n) * A)
        )
    U *= D
    # create Laplacian via 3-point finite-difference method
    ...
    # create plot
    Kronig_Penney_Plot(
        E, V, xvec, steps, n, Case, U, ask_to_save=True, ask_squared=True
    )
    # print output
    output(
        Case,
        ["Well Width (a.u.)", "Well Depth (a.u.)", "Well Separation (a.u.)"],
        [W * 2, -D, B],
        E,
        n,
    )
