...
def infinite_well_input(W=None, n=None):
    if W == None:
        ...
        try:
            n = int(
                input(
                    "Enter the number of wavefunctions you would like to plot.\n\tThis value must be an integer: "
                )
            )
        except ValueError:
            valid_input_error_message()
...
def finite_well_input(W=None, D=None):
    if W == None:
        ...
        try:
            D = -float(
                input(
                    "Enter the depth of your finite well in atomic units (a.u.).\n\tSelect a value between 20 and 500. "
                )
            )
        except ValueError:
            valid_input_error_message()
...
def harmonic_well_input(omega=None, D=None):
    if omega == None:
        ...
        try:
            D = -float(
                input(
                    "Enter the depth of your harmonic well in atomic units(a.u.).\n\tSelect a value between 2 and 15. "
                )
            )
        except ValueError:
            valid_input_error_message()
...
def morse_well_input(omega=None, D=None):
    if omega == None:
        ...
        try:
            D = -float(
                input(
                    "Enter the depth of your morse well in atomic units (a.u.).\n\tSelect a value between 2 and 15. "
                )
            )
        except ValueError:
            valid_input_error_message()
...
def Kronig_Penney_input(A=None, D=None, B=None, num_wells=None):
    if A == None:
        ...
        try:
            D = -float(
                input(
                    "Enter the depth of the repeating finite wells in atomic units (a.u.).\n\tSelect a value between 20 and 500. "
                )
            )
        ...
        try:
            B = float(
                input(
                    "Enter the separation distance of the repeating finite wells in atomic units (a.u.).\n\tSelect a value between 1.0 and 15. "
                )
            )
        ...
        try:
            num_wells = int(
                input(
                    "Enter the number of repeating wells to use.\n\tSelect an odd integer between 3 and 7. "
                )
            )
        except ValueError:
            valid_input_error_message()
...
def ask_to_save_plot(error=False):
    ...
    try:
        image = input(
            "Would you like to save a .png image of your plot? Type yes or no. "
        )
    except:
...
def ask_to_plot_squared(error=False):
    ...
    try:
        sq = input(
            "Would you like to plot the probability density (psi squared) instead of the probability amplitude (psi)? Type yes or no. "
        )
    except:
...
# PLOTTING
########
def infinite_well_plot(
    E, V, xvec, W, steps, n, Case, ask_to_save=False, ask_squared=False
):
    if ask_squared:
    ...
    for i in np.arange(n - 1, -1, -1):
        ...
        wavefunc = ax.plot(
            ...
            V_new[0:steps, i],
            c=color,
            label="E(a.u.)={}".format(np.round(E[i] * 1000) / 1000.0),
        )
    ...
    # add plot legend
    L = plt.legend(bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.0)
    box = ax.get_position()
...
def finite_well_plot(
    E, V, xvec, steps, n, Case, U, ask_to_save=False, ask_squared=False
):
    if ask_squared:
        sq = ask_to_plot_squared()
        if sq == "yes":
    ...
    while Converged is False:
        ...
        while Overlap == 1:
            for i in range(0, n, step):
                ...
                MaxV1 = (
                    np.max(V_copy[0:steps, i - step]) + E_copy[i - step] / ScaleFactor
                )
